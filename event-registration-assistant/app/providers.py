...
def demo_providers(slow: float=0.0)->dict:
    """Deterministic scripts for the demo: one successful registration and one full-event waitlist case."""
    return {
      "supervisor": RoutedMock({
        "AI & Backend": [
          _call("ask_events", question="Find the AI & Backend Engineering Workshop and tell me if I am eligible."),
          _call("ask_registration", request="Register the student for event 1 (AI & Backend Engineering Workshop) and send a confirmation."),
          ModelTurn(text="(mock) You are eligible and your seat is registered. A confirmation message was recorded."),
        ],
        "Cloud Career": [
          _call("ask_events", question="Find the Cloud Career Talk and tell me its seat availability."),
          _call("ask_registration", request="Register the student for event 2 (Cloud Career Talk). If it is full, join the waitlist and confirm."),
          ModelTurn(text="(mock) The Cloud Career Talk is full, so you have been placed on its waitlist and notified."),
        ],
      },slow),
      "events": RoutedMock({
        "AI & Backend": [_call("list_events", text="AI & Backend"), _call("check_eligibility", event_id=1),
                         ModelTurn(text="(mock) Event 1 has 2 seats and the student is eligible.")],
        "Cloud Career": [_call("list_events", text="Cloud Career"), _call("check_eligibility", event_id=2),
                         ModelTurn(text="(mock) Event 2 is currently full; the student is eligible.")],
      },slow),
      "registration": RoutedMock({
        "event 1": [_call("check_registration_limit"), _call("register", event_id=1),
                    _call("notify_student", message="Your AI & Backend Engineering Workshop registration is confirmed."),
                    ModelTurn(text="(mock) Registered for event 1 and sent the confirmation.")],
        "event 2": [_call("check_registration_limit"), _call("register", event_id=2),
                    _call("join_waitlist", event_id=2),
                    _call("notify_student", message="You are on the Cloud Career Talk waitlist."),
                    ModelTurn(text="(mock) Event 2 is full, so I joined the waitlist and sent the confirmation.")],
      },slow),
    }
