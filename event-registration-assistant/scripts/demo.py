"""End-to-end demo. No API key is needed.
...
QUESTIONS=[
 ("22CS045","Find the AI & Backend Engineering Workshop and register me if I am eligible."),
 ("22CS045","Find the Cloud Career Talk and register me. If it is full, join the waitlist."),
]
class Crash(BaseException): pass
def counts(db):
    return f"registrations {db.count('registration')}   waitlist {db.count('waitlist')}   notifications {db.count('notification')}   idempotency keys {db.count('idempotency')}"
def main():
    ...
    for student_id,text in questions:
        thread=store.create_thread(student_id); run_id=store.enqueue(thread,text,providers["supervisor"].model)
        print(f"{CYAN}{student_id}>{RESET} {text}")
        if a.crash:
            real_once=db.once
            def once_then_die(key,tool_name,effect):
                result=real_once(key,tool_name,effect)
                if tool_name=="register": raise Crash()
                return result
        ...
        print(f"{DIM}run {run_id[:8]} {run['status']} after {run['attempts']} attempt(s){RESET}\n")
    print(f"after:  {counts(db)}")
    if a.crash:
        ok=db.count("registration")==3 and db.count("notification")==1
        print(f"{GREEN}PASS: one registration, one notification{RESET}" if ok else f"{RED}FAIL: duplicate side effects{RESET}")
